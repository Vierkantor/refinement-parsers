...
\begin{code}
  wp : (implicit(C : Set)) (implicit(R: C -> Set)) (implicit(a : Set)) ((c : C) -> (R c -> Set) -> Set) ->
    (a -> Set) -> (Free (eff C R) a -> Set)
  wp alg P (Pure x)     = P x
  wp alg P (Step c k)   = alg c λ x -> wp alg (k x) P
\end{code}
...
\end{code}
%endif
Furthermore, we need to reason about the |allSplits| function that is
called by the |match| function. The |allSplitsSound| lemma follows
directly from the definition of |allSplits| and the |consequence| rule
above:
\begin{code}
